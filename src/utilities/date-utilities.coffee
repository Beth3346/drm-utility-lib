...
DrmUtilities.getYearsToDays = (years) -> return (years * 146097) / 400
DrmUtilities.getDaysToYears = (days) -> return (days * 400) / 146097

DrmUtilities.isLeapYear = (year) ->
...
DrmUtilities.daysInYear = (year) => return if @isLeapYear(year) then 366 else 365

DrmUtilities.getDaysInMonth = (month, year) ->
    # returns the number of days in a month
    _month = month + 1
    return new Date(year, _month, 0).getDate()

DrmUtilities.getDayOfWeek = (month, date, year) ->
    # returns the day of the week for a specific date
    _day = new Date year, month, date
    return _day.getDay()

DrmUtilities.getFirstDayOfMonth = (month, year) =>
    return @getDayOfWeek month, 1, year

DrmUtilities.getWeeksInMonth = (month, year) =>
...
DrmUtilities.getMonthWeekNum = (dayNum, day, month, year) ->
...
DrmUtilities.getDatesInWeek = (month, newDate, year) ->
    _firstDay = @getDayOfWeek month, 1, year
    _numberDays = @getDaysInMonth month, year
    _dayShift = if _firstDay is @daysPerWeek then 0 else _firstDay
...
DrmUtilities.getWeekNumber = (month, newDate, year) ->
    self = @
    _weekNum = 1
    weekNums = []
...
DrmUtilities.timeRemainderUtilities =
    ...
    getLeftOverDaysInYear: (now, date) ->
        ...
        _days = _days - getYearsToDays(_years)
